...
TRANSFORMER_LAYER_PARAMS = ['attentions.%i.q_lin.weight', 'attentions.%i.q_lin.bias',
                            ...
                            'projector.gr_linear.weight', 'projector.gr_linear.bias',
                            'projector.gl_linear.weight', 'projector.gl_linear.bias']
...
class TransformerModel(nn.Module):
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None,
        ...
        if self.is_encoder and self.select_attn and image_langs is not None:
            encode_out = tensor.clone()
            ...
            if self.vse:
                sim_text_visual = torch.cosine_similarity(select, encode_out, dim=2)
                sim_visual_text = torch.cosine_similarity(encode_out, select, dim=2)

                cl_labels = torch.arange(sim_visual_text.size(0), device=sim_visual_text.device)

                loss_s2t = F.cross_entropy(sim_text_visual, cl_labels, reduction='mean')
                loss_t2s = F.cross_entropy(sim_visual_text, cl_labels, reduction='mean')
                loss_cl = (loss_s2t + loss_t2s) / 2

                img_text = select.view(-1, self.batch_size)
                text_img = encode_out.view(-1, self.batch_size)
                feats_student = F.normalize(text_img, dim=-1)
                feats_teacher = F.normalize(img_text, dim=-1)
                loss_ms = self.loss_fct(feats_student, feats_teacher)

                loss_vse = loss_cl + loss_ms
    ...
    def generate(self, src_enc, src_len, tgt_lang_id, max_len=200, sample_temperature=None):
        ...
        while cur_len < max_len:
            ...
            # compute word scores
            tensor, _ = self.forward('fwd', x=generated[:cur_len], lengths=gen_len, positions=positions[:cur_len],
