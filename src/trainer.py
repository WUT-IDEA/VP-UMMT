...
class Trainer(object):
    ...
    def vmlm_step(self, lang1, lang2, lambda_coeff, iter):
        ...
        # generate batch / select words to predict
        x, lengths, positions, langs, img_boxes, re_img_feats, gr_img_feats, gl_img_feats, img_labels = \
        ...
        #img_boxes, re_img_feats, img_y, img_pred_mask, img_mask_pos = self.mask_out_image(img_boxes, re_img_feats, img_labels)
        ...
        tensor, loss_vse = model('fwd', x=x, lengths=lengths, positions=positions, langs=langs, causal=False,
        ...
        loss = text_loss# + img_loss + loss_vse
...
class EncDecTrainer(Trainer):
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        ...
        enc1, loss_vse = self.encoder('fwd', x=x1, lengths=len1, langs=langs1, causal=False)
        enc1 = enc1.transpose(0, 1)

        # decode target sentence
        dec2, _ = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)

        # loss
        _, loss = self.decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=False)
        self.stats[('AE-%s' % lang1) if lang1 == lang2 else ('MT-%s-%s' % (lang1, lang2))].append(loss.item())
        loss = lambda_coeff * loss + loss_vse
    ...
    def vae_step(self, lang1, lang2, lambda_coeff):
        ...
        # encode source sentence
        enc1, loss_vse = self.encoder('fwd', x=x1, lengths=len1, langs=langs1, causal=False, image_langs=img_langs,
        ...
        # decode target sentence
        dec2, _ = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=src_len)
    ...
    def vbt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        with (torch.no_grad()):
            ...
            # encode source sentence and translate it
            enc1, _ = _encoder('fwd', x=x1, lengths=len1, langs=langs1, causal=False, image_langs=img_langs,
                            img_boxes=img_boxes, re_img_feats=re_img_feats, gr_img_feats=gr_img_feats,
                            gl_img_feats=gl_img_feats)
        ...
        # encode generate sentence
        enc2, loss_vse = self.encoder('fwd', x=x2, lengths=len2, langs=langs2, causal=False, image_langs=img_langs,
                            img_boxes=img_boxes, re_img_feats=re_img_feats, gr_img_feats=gr_img_feats,
                            gl_img_feats=gl_img_feats)
        ...
        # decode original sentence
        dec3, _ = self.decoder('fwd', x=x1, lengths=len1, langs=langs1, causal=True, src_enc=enc2, src_len=src_len2)
