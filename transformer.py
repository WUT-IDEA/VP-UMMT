...
class TransformerModel(nn.Module):
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None,
        ...
        if img_feats is not None and image_langs is not None:
            ...
            #     img = image.view(-1, self.dim)
            #     feats_student = F.normalize(text, dim=-1)
            #     feats_teacher = F.normalize(img, dim=-1)
            #     logits_s2t = feats_student @ feats_teacher.t() / temp
            #     logits_t2s = feats_teacher @ feats_student.t() / temp
            #
            #     cl_labels = torch.arange(logits_s2t.size(0), device=tensor.device)
            #     #
            #     loss_s2t = F.cross_entropy(logits_s2t, cl_labels, reduction='mean')
            #     loss_t2s = F.cross_entropy(logits_t2s.t(), cl_labels, reduction='mean')
            #     loss_cl = (loss_s2t + loss_t2s) / 2
            #     #loss_vse = loss_cl
            #     # # mse
            #     #loss_mse = self.loss_fct(logits_s2t, logits_t2s.t())
            #     # print('22222222222222', cl_labels)
            #
            #     # MSE LOSS
            #     img_text = torch.cat((feats, tensor), dim=1)
            #     text_img = torch.cat((tensor, feats), dim=1)
            #     img_text = img_text.view(-1, self.batch_size)
            #     text_img = text_img.view(-1, self.batch_size)
            #     feats_student = F.normalize(text_img, dim=-1)
            #     feats_teacher = F.normalize(img_text, dim=-1)
            #     # loss_mse = self.loss_fct(feats_student, feats_teacher)
            #
            #     cl_labels = torch.arange(feats_student.size(0),  dtype=torch.long).cuda()
            #     loss_s2t = F.cross_entropy(feats_student, cl_labels, reduction='mean')
            #     loss_t2s = F.cross_entropy(feats_teacher, cl_labels, reduction='mean')
            #     loss_cl = (loss_s2t + loss_t2s) / 2
            #     loss_vse = loss_cl
        ...
        if self.is_encoder and self.select_attn and img_feats is not None and image_langs is not None:
            ...
            if self.vse:
                ...
                # text = tensor.clone()
                # image = feats.clone()
                # text = tensor.view(-1, self.dim)
                # img = feats.view(-1, self.dim)
                # feats_student = F.normalize(text, dim=-1)
                # feats_teacher = F.normalize(img, dim=-1)
                # logits_s2t = feats_student @ feats_teacher.t() / temp
                # logits_t2s = feats_teacher @ feats_student.t() / temp
                ...
                text = tensor.clone()
                image = fused.clone()
                img_text = image.view(-1, self.batch_size)
                text_img = text.view(-1, self.batch_size)
                feats_student = F.normalize(text_img, dim=-1)
                feats_teacher = F.normalize(img_text, dim=-1)
                loss_vse = self.loss_fct(feats_student, feats_teacher)
    ...
    def generate(self, src_enc, src_len, tgt_lang_id, max_len=200, sample_temperature=None):
        ...
        while cur_len < max_len:
            ...
            # compute word scores
            tensor, _ = self.forward('fwd', x=generated[:cur_len], lengths=gen_len,
    ...
    def generate_beam(self, src_enc, src_len, tgt_lang_id, beam_size, length_penalty, early_stopping, max_len=200):
        ...
        while cur_len < max_len:
            ...
            # compute word scores
            tensor, _ = self.forward('fwd', x=generated[:cur_len], lengths=src_len.new(bs * beam_size).fill_(cur_len),
                                  positions=positions[:cur_len], langs=langs[:cur_len], causal=True,
                                  src_enc=src_enc, src_len=src_len, cache=cache)
